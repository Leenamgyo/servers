...
- Added `Role` enum (`ROLE_USER`, `ROLE_ADMIN`).
- Added `CustomUserDetailsService` to load user roles from the database.
- Updated `Member` entity and `MemberRequestDto` to support roles.
- Added `/api/user` and `/api/admin` test endpoints.
- Restricted `/api/admin` to users with `ROLE_ADMIN`.
